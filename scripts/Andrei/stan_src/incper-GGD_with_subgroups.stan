...
data {
    ...
    int<lower = 2> V; // number of vaccination status
    int<lower = 2> A; // number of age groups
    array[N] int<lower = 1, upper = V> vaccination_status_record;
    array[N] int<lower = 1, upper = A> age_group_record;
...
transformed parameters {
    ...
    vector[N] exposure = fma(to_vector(exposureR) - to_vector(exposureL_realized), 
      exposure_raw, to_vector(exposureL_realized)),
      onset;
    for (n in 1:N) {
...
generated quantities {
    ...
    vector<lower = 0>[V] mean_interval_vac;
    {
        for (v in 1 : V) {
            int n_vac = 0;
            real incper_vac_sum = 0.0;
            for (n in 1 : N) {
                if (vaccination_status_record[n] == v) {
                    incper_vac_sum += time_interval[n];
                    n_vac += 1;
                }
            }
            mean_interval_vac[v] = incper_vac_sum / n_vac;
        }
    }

    vector<lower = 0>[A] mean_interval_age;
    {
        for (age in 1 : A) {
            int n_age = 0;
            real incper_age_sum = 0.0;
            for (n in 1 : N) {
                if (age_group_record[n] == age) {
                    incper_age_sum += time_interval[n];
                    n_age += 1;
                }
            }
            mean_interval_age[age] = incper_age_sum / n_age;
        }
    }
}